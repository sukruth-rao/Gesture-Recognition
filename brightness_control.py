			...
			if results.multi_hand_landmarks:
				...
				if 200 < area < 1000:
					...
					if 21 < brightnessPercent < 50:
						cv2.rectangle(image, (w - 50, int(brightnessBar)), (w - 80, 400), (0, 255, 0), cv2.FILLED)
						cv2.putText(image, f'{int(brightnessPercent)} %', (w - 100, 450), cv2.FONT_HERSHEY_COMPLEX,
				  1, (0, 255, 0), 2)
					elif 51 < brightnessPercent < 80:
						cv2.rectangle(image, (w - 50, int(brightnessBar)), (w - 80, 400), (0, 255, 255), cv2.FILLED)
						cv2.putText(image, f'{int(brightnessPercent)} %', (w - 100, 450), cv2.FONT_HERSHEY_COMPLEX,
				  1, (0, 255, 255), 2)
					elif brightnessPercent > 81:
						cv2.rectangle(image, (w - 50, int(brightnessBar)), (w - 80, 400), (0, 0, 255), cv2.FILLED)
						cv2.putText(image, f'{int(brightnessPercent)} %', (w - 100, 450), cv2.FONT_HERSHEY_COMPLEX,
				  1, (0, 0, 255), 2)
					elif brightnessPercent < 20:
						cv2.rectangle(image, (w - 50, int(brightnessBar)), (w - 80, 400), (255, 255, 0), cv2.FILLED)
						cv2.putText(image, f'{int(brightnessPercent)} %', (w - 100, 450), cv2.FONT_HERSHEY_COMPLEX,
				  1, (255, 255, 0), 2)

					current_brightness = int(sbc.get_brightness()[0])
					cv2.putText(image, f'Current brightness: {int(current_brightness)}', (0, 60), cv2.FONT_HERSHEY_COMPLEX,
				 1, (255, 255, 255), 2)
					...
					if fCount[3] == 0 and fCount[2] == 1 and fCount[1] == 1 and fCount[0] == 1:
						...
						cv2.putText(image, 'brightness Set', (0, 90), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 0, 255), 2)
						colorVol = (0, 255, 0)
